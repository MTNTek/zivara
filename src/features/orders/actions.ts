  ...
  try {
    ...
    if (customerEmail) {
      ...
      sendOrderConfirmationEmail({
        ...
        trackingUrl: `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/track/${result.orderNumber}`,
      }).catch((error) => {
      ...
      if (validated.status === 'shipped') {
        ...
        sendShippingNotificationEmail({
          ...
          estimatedDeliveryDate: order.estimatedDeliveryDate
            ...
            : undefined,
          trackingUrl: `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/track/${order.orderNumber}`,
        }).catch((error) => {
